...
let role = 3; // Số lượng trang tối đa hiển thị
        ...
        .then(data => {
            ...
            data.items.forEach(user => {
                ...
                row.innerHTML = `
                    ...
                    <td>${phoneCell}</td>
                    <td>${addressCell}</td>
                    <td>${isDeleted === true ? 'Disabled' : 'Active'}</td>  <!-- Show "Disabled" or "Active" text -->   
                    <td>
                        <ul>
                            <li>
                                <a href="StaffController?staffAction=staffDetail&staffId=${user.userId}">
                                    <i class="ri-eye-line"></i>
                                </a>
                            </li>
                            <li>
                            <a href="javascript:void(0);" onclick="toggleUserStatus(${user.userId}, ${user.isDeleted})">
                            <i class="${user.isDeleted ? 'ri-check-line' : 'ri-close-circle-line'}"></i>
                            </a>
                             </li>
...
function toggleUserStatus(userId, status) {
    let url;
    if (status == false) {
        url = `/webmenu/StaffController?staffAction=updateStatusStaff&staffId=` + userId + `&status=` + true;
    } else {
        url = `/webmenu/StaffController?staffAction=updateStatusStaff&staffId=` + userId + `&status=` + false;
    }
    fetch(url) // Gọi API với URL đã tạo
        .then(response => response.json()) // Chuyển đổi phản hồi thành JSON
        .then(data => {
            alert("Update Status Staff Successfully")
            loadUserList(currentPage)
        }).catch(error => console.error('Error:', error)); // Xử lý lỗi khi gọi API
}